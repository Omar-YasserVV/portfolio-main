...
const MobileNav = ({
  ...
  useMotionValueEvent(scrollYProgress, "change", (current) => {
    if (typeof current === "number") {
      ...
      // If user hasn't scrolled much, always show nav
      if (current < 0.05) {
        setVisible(true);
        return;
      }

      // Only apply scroll-based hiding after user has scrolled
      if (hasScrolled) {
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          "flex max-w-fit md:min-w-[70vw] lg:min-w-fit fixed z-[5000] top-2 inset-x-0 mx-auto px-4 py-4 rounded-lg border border-black/.1 shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] items-center justify-center space-x-3",
          className
        )}
        style={{
